...
public class OrderRepositoryImpl implements OrderRepository {
	...
	public PedidoRespuesta guardaPedido(PedidoConsulta consulta) throws Exception {
		...
		log.info("Entrando a mÃ©todo guardaPedido");
		...
		try {
			...
			cstmt = e.prepareCall("{? = call PPVMX.PCK_MAGENTO.F_GET_PEDIDO_MAGENTO(?)}");
			cstmt.registerOutParameter(1, 4);
			cstmt.setString(2, pedidoAlta.getPedidoMagento());
			cstmt.execute();
			Long pRegresa = Long.valueOf(cstmt.getLong(1));
			if (pRegresa.longValue() > 0L) {
				errorConocido = true;
				respuesta.setCodigo(99);
				respuesta.setMensaje("El pedido Magento ya fue replicado al Soft, Pedido Soft: " + pRegresa);
				log.info("**************** El pedido Magento ya fue replicado al Soft, Pedido Soft: " + pRegresa);
				PedidoRespuesta arg45 = respuesta;
				return arg45;
			} else {
				tx = sess.beginTransaction();
				Integer[] idDirecciones = new Integer[] { pedidoAlta.getDireccionEnvio().getIdDireccionPs(),
						pedidoAlta.getDireccionPago().getIdDireccionPs() };
				Iterator altaPed;
					...
					if (pedidoAlta.getFormasPagos().getItem().size() == 0) {
						...
						label874: while (true) {
							while (true) {
								...
								if (((FormaPagoPedido) pedidoAlta.getFormasPagos().getItem().get(0))
										.getFormaPago() != null
										&& ((FormaPagoPedido) pedidoAlta.getFormasPagos().getItem().get(0))
												.getCantidad() != null) {
				...
				if (errorConocido) {
					PedidoRespuesta arg49 = respuesta;
					return arg49;
				} else {
					for (int arg46 = 0; arg46 < idDirecciones.length; ++arg46) {
						...
						if (arg52 == null) {
							...
							log.info(arg56);
							log.info(pedidoAlta.getSocioIdPs());
							...
							if (pedidoAlta.getDireccionEnvio().getPaisId() != null) {
								arg56.setPaCveN(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getPaisId()
										: pedidoAlta.getDireccionPago().getPaisId());
							} else {
								errorConocido = true;
								respuesta.setCodigo(Constantes.COD_ERROR_FORMATO_DIRECCION);
							...
							if (pedidoAlta.getDireccionEnvio().getEstadoId() != null) {
								arg56.setEdCveN(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getEstadoId()
										: pedidoAlta.getDireccionPago().getEstadoId());
							} else {
								errorConocido = true;
								respuesta.setCodigo(Constantes.COD_ERROR_FORMATO_DIRECCION);
							...
							arg56.setMuCveN(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getMunicipioId()
									: pedidoAlta.getDireccionPago().getMunicipioId());
							if (pedidoAlta.getDireccionEnvio().getCiudad() != null
									&& !pedidoAlta.getDireccionEnvio().getCiudad().trim().equals("")) {
								arg56.setSodCdStr(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getCiudad().toUpperCase()
										: pedidoAlta.getDireccionPago().getCiudad().toUpperCase());
							} else {
								errorConocido = true;
								respuesta.setCodigo(Constantes.COD_ERROR_FORMATO_DIRECCION);
							...
							if (pedidoAlta.getDireccionEnvio().getCalle() != null
									&& !pedidoAlta.getDireccionEnvio().getCalle().trim().equals("")) {
								arg56.setSodCalleStr(
										arg46 == 0 ? pedidoAlta.getDireccionEnvio().getCalle().toUpperCase()
												: pedidoAlta.getDireccionPago().getCalle().toUpperCase());
							} else {
								errorConocido = true;
								respuesta.setCodigo(Constantes.COD_ERROR_FORMATO_DIRECCION);
							...
							if (pedidoAlta.getDireccionEnvio().getCodigoPostal() != null
									&& !pedidoAlta.getDireccionEnvio().getCodigoPostal().trim().equals("")) {
								arg56.setSodCpStr(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getCodigoPostal()
										: pedidoAlta.getDireccionPago().getCodigoPostal());
							} else {
								errorConocido = true;
								respuesta.setCodigo(Constantes.COD_ERROR_FORMATO_DIRECCION);
								respuesta.setMensaje("El CÃ³digo postal es obligario.");
							}

							if (pedidoAlta.getDireccionEnvio().getNumero() != null
									&& !pedidoAlta.getDireccionEnvio().getNumero().trim().equals("")) {
							...
							if (pedidoAlta.getDireccionEnvio().getColonia() != null
									&& !pedidoAlta.getDireccionEnvio().getColonia().trim().equals("")) {
							...
							if (pedidoAlta.getDireccionEnvio().getTelefono1() != null
									&& !pedidoAlta.getDireccionEnvio().getTelefono1().trim().equals("")) {
								arg56.setSodTel1Str(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getTelefono1()
										: pedidoAlta.getDireccionPago().getTelefono1());
							}

							if (pedidoAlta.getDireccionEnvio().getTelefono2() != null
									&& !pedidoAlta.getDireccionEnvio().getTelefono2().equals("")) {
								arg56.setSodTel2Str(arg46 == 0 ? pedidoAlta.getDireccionEnvio().getTelefono2()
										: pedidoAlta.getDireccionPago().getTelefono2());
							}

							if (pedidoAlta.getDireccionEnvio().getObservaciones() != null
									&& !pedidoAlta.getDireccionEnvio().getObservaciones().trim().equals("")) {
								log.error((new Gson()).toJson(pedidoAlta.getDireccionEnvio()));
								log.error((new Gson()).toJson(pedidoAlta.getDireccionPago()));
								if (pedidoAlta.getDireccionPago().getObservaciones() == null) {
									pedidoAlta.getDireccionPago().setObservaciones("");
								}

								arg56.setSodObsStr(
										arg46 == 0 ? pedidoAlta.getDireccionEnvio().getObservaciones().toUpperCase()
												: pedidoAlta.getDireccionPago().getObservaciones().toUpperCase());
							}

							if (errorConocido) {
								PedidoRespuesta arg54 = respuesta;
								return arg54;
							}

							log.info((new Gson()).toJson(arg56));
							sess.save(arg56);
						} else {
							log.info("Existe");
							switch (arg46) {
							...
							default:
								...
								if (pedidoAlta.getDireccionPago().getMunicipioId() != null
										&& pedidoAlta.getDireccionPago().getMunicipioId().intValue() > 0) {
									arg52.setMuCveN(pedidoAlta.getDireccionPago().getMunicipioId());
								}

								if (pedidoAlta.getDireccionPago().getCiudad() != null
										&& !pedidoAlta.getDireccionPago().getCiudad().trim().equals("")) {
									arg52.setSodCdStr(pedidoAlta.getDireccionPago().getCiudad().toUpperCase());
								}

								if (pedidoAlta.getDireccionPago().getCalle() != null
										&& !pedidoAlta.getDireccionPago().getCalle().trim().equals("")) {
									arg52.setSodCalleStr(pedidoAlta.getDireccionPago().getCalle().toUpperCase());
								}

								if (pedidoAlta.getDireccionPago().getCodigoPostal() != null
										&& !pedidoAlta.getDireccionPago().getCodigoPostal().trim().equals("")) {
									arg52.setSodCpStr(pedidoAlta.getDireccionPago().getCodigoPostal());
								}

								if (pedidoAlta.getDireccionPago().getNumero() != null
										&& !pedidoAlta.getDireccionPago().getNumero().trim().equals("")) {
									arg52.setSodNumStr(pedidoAlta.getDireccionPago().getNumero().toUpperCase());
								}

								if (pedidoAlta.getDireccionPago().getColonia() != null
										&& !pedidoAlta.getDireccionPago().getColonia().trim().equals("")) {
									arg52.setSodColStr(pedidoAlta.getDireccionPago().getColonia().toUpperCase());
								}

								if (pedidoAlta.getDireccionPago().getTelefono1() != null
										&& !pedidoAlta.getDireccionPago().getTelefono1().trim().equals("")) {
									arg52.setSodTel1Str(pedidoAlta.getDireccionPago().getTelefono1());
								}

								if (pedidoAlta.getDireccionPago().getTelefono2() != null
										&& !pedidoAlta.getDireccionPago().getTelefono2().equals("")) {
									arg52.setSodTel2Str(pedidoAlta.getDireccionPago().getTelefono2());
								}

								if (pedidoAlta.getDireccionPago().getObservaciones() != null
										&& !pedidoAlta.getDireccionPago().getObservaciones().trim().equals("")) {
					...
					while (altaPedDetalles.hasNext()) {
						...
						int consecutivoDep = ((ArticuloPedido) pedidoAlta.getItems().getItem().get(numArt))
								.getCantidad().intValue();
						++numArt;

						for (altaTmkDeposito = 0; altaTmkDeposito < consecutivoDep; ++altaTmkDeposito) {
							++intTotArt;
							dblTotalPed = Double.valueOf(
									dblTotalPed.doubleValue() + conseDepNum.getArticulo().getPrecio().doubleValue());
					...
					while (arg59.hasNext()) {
						...
						altaTmkDeposito = ((ArticuloPedido) pedidoAlta.getItems().getItem().get(numArt)).getCantidad()
								.intValue();
						++numArt;
					...
					if (pedidoAlta.getFormasPagos() != null) {
						...
						while (arg65.hasNext()) {
							...
							if (fPagoEcomm.getFpeCveStr().equals("F")) {
								...
								this.guardarDoctos(value, respuesta.getFormaPagoPedido().getCantidad(),
										arg48.getSoIdStr(), sess);
							} else if (fPagoEcomm.getFpeCveStr().equals("S")) {
								respuesta.setBanderaSaldo(true);
								respuesta.getFormaPagoPedido().setFormaPago(cveFormaPago);
								respuesta.getFormaPagoPedido().setCantidad(formaPago.getCantidad());
								this.guardarSaldo(value, respuesta.getFormaPagoPedido().getCantidad(),
										arg48.getSoIdStr(), sess);
							} else {
								if (!fPagoEcomm.getFpeCveStr().equals("K")) {
									...
									arg64.setDepRefStr(formaPago.getReferencia().equals("") ? "009999"
											: formaPago.getReferencia());
					...
					log.info("Numero de Socio: " + pedidoAlta.getPedidoMagento());
					log.info("Numero de Pedido Magento: " + consulta.getPedido().getPedidoMagento());
					log.info("Numero de Pedido Soft   : " + value);
					cstmt = e.prepareCall("{? = call PPVMX.PCK_MAGENTO.F_INSERTA_PEDIDO_MAGENTO(?,?)}");
					cstmt.registerOutParameter(1, 4);
					cstmt.setLong(2, value.longValue());
					cstmt.setString(3, pedidoAlta.getPedidoMagento());
					cstmt.execute();
					pRegresa = Long.valueOf(cstmt.getLong(1));
					if (pRegresa.longValue() == -1L) {
						log.info("********** ERROR EN, INSERTA PEDIDO MAGENTO EN EL SOFT **********");
						log.info("Verificar tabla PS_BIT_WEB");
					}
